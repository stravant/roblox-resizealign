...
function FreeformDragger:_updateGeometric()
	...
	if dragTarget then
		...
		if dragTarget.unionPart then
			for _, union in ipairs(self._draggerToolModel._selectionInfo:getUnionParts()) do
				if union.Parent ~= dragTarget.unionPart.Parent then
					union.Parent = dragTarget.unionPart.Parent
				end
			end
		end
	end
